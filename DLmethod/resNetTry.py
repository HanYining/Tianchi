...
if __name__ == "__main__":
    ...
    train_data, train_label = SMOTE(
        random_state=0,kind='borderline1',ratio={0:4429,1:4429,2:7000,3:7000}
    ).fit_sample(
        train_data, train_label
    )
    print(Counter(train_label))
    ...
    # initialize the net
    ctx = mx.gpu()
    # X = nd.array(train_data[:300, :], ctx=ctx)
    ...
    trainer = gluon.Trainer(net.collect_params(), 'adagrad', {'learning_rate': 0.05,'wd': 0.01})
